...
Sub OpenAI_Completion()
    ...
    ' Get the selected text in the active document
    Dim selectedText As String
    selectedText = Selection.Text

    ' Check if there is any selected text
    If Trim(selectedText) <> "" Then
        ' Clean selected text to avoid parsing error in JSON payload
        selectedText = CleanJSONString(selectedText)
    Else
        MsgBox "Please select some text in the document before executing the macro", vbCritical, "Empty Selection"
        Exit Sub
    End If
    ...
    If httpRequest.Status = 200 Then
        ...
        Selection.TypeText Text:=completion

        ' Show completion message
        MsgBox "OpenAI completion request processed successfully. Result has been inserted into the document.", vbInformation, "OpenAI Request Completed"
    Else
